...
export const sendConnectionRequest=async(req,res)=>{
    try{
        const {userId}=req.auth()
        const {id}=req.body;
        //Check if user has sent more that 20 request in the last 24 hrs
        ...
        const connectionRequests=await Connections.find(
            ...
            {
                return res.json({success:false, message:'You have sent more that 20 connection requests in the last 24 hours'})
            }
