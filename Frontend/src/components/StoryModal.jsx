...
const StoryModal = ({setShowModal,fetchStories}) => {
        ...
        if(file)
        {
            if(file.type.startsWith("video")){
                if(file.size>MAX_VIDEO_SIZE *1024*1024){
                    toast.error(`Video file size cannot exceed ${MAX_VIDEO_DURATION} mb.`)
                    setMedia(null)
                    setPreviewUrl(null)
                    return;
    ...
    backdrop-blur text-white flex item-center justify-center p-4 '>
        <div className='w-full max-w-md'>
                ...
                <div className='rounded-lg h-96 flex items-center justify-center relative' style={{backgroundColor:background}}>
                    ...
                    {mode==='text' && (
                            <textarea className='bg-transparent text-white placeholder-gray-400  w-full h-full
                            p-6 text-lg resize-none focus:outline-none'
                             placeholder="what's on your mind?"
                             onChange={(e)=>setText(e.target.value)} value={text}/>
                    )}
                    {/* if it contains image or video  */}
                    {
                ...
                <div className='flex mt-4 gap-2'>
                    {
                        bgColors.map((color)=>(<button key={color} className='
                            w-6 h-6 rounded-full ring cursor-pointer' style={{backgroundColor:color}}
                            onClick={()=>setBackground(color)}/>))
                    }
                </div>
                <div className='flex gap-2 mt-4'>
                    <button onClick={()=>{setMode('text');setMedia(null);setPreviewUrl(none)}}className={`flex-1 flex items-center justify-center gap-2 cursor-pointer
